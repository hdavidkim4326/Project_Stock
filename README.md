...
투자를 시작하면서 "매달 같은 금액을 넣는 게 나을까, 시장 상황에 따라 조절하는 게 나을까?"라는 질문에 답을 찾고 싶었습니다. 이론만으로는 부족해서, 실제 주가 데이터를 기반으로 두 전략을 **직접 비교하고 시각화**할 수 있는 도구를 만들었습니다.

### 왜 만들었나?

- 📊 **데이터 기반 투자 판단** — 감이 아닌 숫자로 전략을 비교하고 싶었습니다
- 🔄 **실시간 시뮬레이션** — 비중, 금액, 시작 시점을 슬라이더로 바꾸면 즉시 결과가 바뀌는 인터랙티브 경험
- 📚 **학습 목적** — 풀스택 웹 개발(React + FastAPI)과 금융 데이터 처리를 동시에 연습
